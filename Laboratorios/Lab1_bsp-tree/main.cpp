...
void testTreeStructureValidity() {
    ...
    for (int i = 0; i < numPolygons; ++i) {
        ...
        if (poly.getVertices().empty()) {
            std::cerr << "[WARN] Polígono generado está vacío. Iteración: " << i << "\n";
            continue;
        }
        tree.insert(poly);
    }

    std::cout << "Inserción de polígonos terminada.\n";

    auto nodes = tree.getAllNodes();
    std::cout << "Total de nodos obtenidos: " << nodes.size() << "\n";

    if (nodes.empty()) {
        std::cerr << "[ERROR] No se insertaron nodos en el árbol.\n";
        return;
    }

    for (const BSPNode<NType>* node : nodes) {
        if (!node) {
            std::cerr << "[ERROR] Nodo nulo encontrado.\n";
            continue;
        }

        std::cout << "Verificando nodo...\n";

        const BSPNode<NType>* frontNode = node->getFront();
        if (frontNode) {
            bool validFront = checkSubtreeValidity(frontNode, node->getPartition(), true);
            assert(validFront && "Subárbol front inválido.");
        }

        const BSPNode<NType>* backNode = node->getBack();
        if (backNode) {
            bool validBack = checkSubtreeValidity(backNode, node->getPartition(), false);
            assert(validBack && "Subárbol back inválido.");
        }
    }
